...
parser.add_argument('--train_list', type=str, default='./data/train_list.txt', help='path to train dataset list file')
parser.add_argument('--eval_list', type=str, default='./data/test_list.txt', help='path to evaluation dataset list file')
...
parser.add_argument('--span', type=float, default=0.9, help='x and y in grid are within the range of [-span, span]')
parser.add_argument('--grid_H', type=int, default=4, help='the number of fiducial points equals to grid_H * grid_W')
parser.add_argument('--grid_W', type=int, default=5, help='the number of fiducial points equals to grid_H * grid_W')
...
parser.add_argument('--learning_rate', type=float, default=0.0001, help='learning rate for Critic, default=0.0001')
parser.add_argument('--transformer', type=str, default='', help='path to transformer (to continue training)')
parser.add_argument('--encoder', type=str, default='', help='path to encoder (to continue training)')
parser.add_argument('--decoder', type=str, default='', help='path to decoder (to continue training)')
parser.add_argument('--model', default='./model/rare/', help='Where to store samples and models')
...
# load alphabet
with open('./data/char_std_5990.txt', "rt", encoding="utf-8") as f: # encoding='gb18030'
    data = f.readlines()
...
def train(image, transformer, encoder, decoder, criterion, train_loader, teach_forcing_prob=1):
    # optimizer
    transformer_optimizer = torch.optim.Adam(transformer.parameters(), lr=cfg.learning_rate, betas=(0.5, 0.999))
    ...
    for epoch in range(cfg.num_epochs):
        ...
        for i in range(len(train_loader)):
            ...
            for t, e, d in zip(transformer.parameters(), encoder.parameters(), decoder.parameters()):
            ...
            # STN
            stn_outputs = transformer(image)
            # CNN + BiLSTM
            encoder_outputs = encoder(stn_outputs)
            ...
            if i % 10 == 0:
                print('[Epoch {0}/{1}] [Batch {2}/{3}] Loss: {4}'.format(
                    epoch, cfg.num_epochs, i, len(train_loader), loss_avg.val()))
                loss_avg.reset()

        # save checkpoint
        torch.save(transformer.state_dict(), '{0}/transformer_{1}.pth'.format(cfg.model, epoch))
...
def evaluate(image, transformer, encoder, decoder, data_loader, max_eval_iter=100):
...
def main():
    ...
    # create test dataset
    test_dataset = dataset.TextLineDataset(
        text_line_file=cfg.eval_list, transform=dataset.ResizeNormalize(img_width=cfg.img_width, img_height=cfg.img_height))
    ...
    transformer = stn.STN(
        channel_size=3, imgH=cfg.img_height, imgW=cfg.img_width,
        span_H=cfg.span, span_W=cfg.span, grid_H=cfg.grid_H, grid_W=cfg.grid_W)
    # create rare/seq2seq/attention network
    ...
    print(encoder)
    print(decoder)
    transformer.apply(utils.weights_init)
    encoder.apply(utils.weights_init)
    decoder.apply(utils.weights_init)
    if cfg.transformer:
        print('loading pretrained transformer model from %s' % cfg.transformer)
        transformer.load_state_dict(torch.load(cfg.transformer))
    if cfg.encoder:
    ...
    # train rare
    train(image, transformer, encoder, decoder, criterion, train_loader, teach_forcing_prob=cfg.teaching_forcing_prob)

    # do evaluation after training
    evaluate(image, transformer, encoder, decoder, test_loader, max_eval_iter=100)
